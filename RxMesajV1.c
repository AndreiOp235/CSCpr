		...
		{																										
			...
			// adreasa_hw_dest = ascii2bin(ptr);
				do{
					...
					if(timeout){			// (sau de la nodul care detine jetonul)
						return TMO;					// S: timeout, terminare receptie, nodul va deveni master										// sau va anunta ca s-a pierdut jetonul si va regenera jetonul
					}
					if(ch == ':'){		// S: asteapta sincronizarea cu inceputul mesajului
